...
void SSaveFileManagerTab::Construct(const FArguments& InArgs) {
    CurrentSaveDirectory = FPaths::ConvertRelativePathToFull(FFrameworkSaveFileEditorModule::GetDefaultSaveDirectory());
...
void SSaveFileManagerTab::UpdateSaveFileList() {
    ...
    FFrameworkSaveFileEditorModule::GetSaveGameNames(NewSaveFiles, CurrentSaveDirectory);
...
FReply SSaveFileManagerTab::DeleteAllSaveFilesClick() {
    ...
    for (FString& File : SaveFiles) {
        ...
        TWeakPtr<SSaveFileEditorTab> CurrentSaveFileEditorTab = FFrameworkSaveFileEditorModule::Get().CurrentSaveFileEditorTab;
...
bool SSaveFileEntry::IsButtonsEnabled() const {
    return !FFrameworkSaveFileEditorModule::Get().IsGameStarted();
}

FReply SSaveFileEntry::EditSaveFileClick() {
    TSharedPtr<FTabManager> SaveEditorTabManager = FFrameworkSaveFileEditorModule::Get().GetSaveEditorTabManager();
    ...
    TSharedPtr<SSaveFileEditorTab> SafeFileEditorTab = FFrameworkSaveFileEditorModule::Get().CurrentSaveFileEditorTab.Pin();
