...
void SSaveFileEditorTab::Construct(const FArguments& InArgs) {
    ...
    bIsEditable = !FFrameworkSaveFileEditorModule::Get().IsGameStarted();
}
...
SSaveFileEditorTab::~SSaveFileEditorTab() {
    ...
    ClearEditedObject();
    FFrameworkSaveFileEditorModule::Get().CurrentSaveFileEditorTab.Reset();
}
...
TSharedRef<class SDockTab> SSaveFileEditorTab::OnSpawnContentTab(const FSpawnTabArgs& SpawnTabArgs) {
    ...
    FFrameworkSaveFileEditorModule::Get().CurrentSaveFileEditorTab = SaveFileEditor;
...
void SSaveFileEditorTab::OnPieWorldStarted(bool bIsSimulated) {
    TSharedPtr<FTabManager> TabManagerPtr = FFrameworkSaveFileEditorModule::Get().GetSaveEditorTabManager();
    if (FFrameworkSaveFileEditorModule::IsTabVisible(TabManagerPtr, GetTabName()) && PropertyWidget.IsValid()) {
        bIsEditable = false;
    }
}
