#include "FrameworkSaveFileEditor.h"
...
#define LOCTEXT_NAMESPACE "FFrameworkSaveFileEditorModule"

void FFrameworkSaveFileEditorModule::StartupModule() {
    ...
    PluginCommands->MapAction(
        ...
        FExecuteAction::CreateRaw(this, &FFrameworkSaveFileEditorModule::PluginButtonClicked),
        FCanExecuteAction(),
        FIsActionChecked::CreateStatic(
            &FFrameworkSaveFileEditorModule::IsTabVisibleGlobal,
            SaveFileEditorTabName));

    // Register hubber utils menu
    UToolMenus::RegisterStartupCallback(FSimpleMulticastDelegate::FDelegate::CreateRaw(this, &FFrameworkSaveFileEditorModule::RegisterMenus));

    // Register plugin tab
    FGlobalTabmanager::Get()
        ->RegisterNomadTabSpawner(SaveFileEditorTabName, FOnSpawnTab::CreateRaw(this, &FFrameworkSaveFileEditorModule::OnSpawnPluginTab))
        .SetDisplayName(LOCTEXT("FSaveFileEditorTabTitle", "Save Files Editor"))
    ...
    PostPIEStartedHandle = FEditorDelegates::PostPIEStarted.AddRaw(this, &FFrameworkSaveFileEditorModule::OnPieWorldStarted);
    EndPIEHandle = FEditorDelegates::EndPIE.AddRaw(this, &FFrameworkSaveFileEditorModule::OnPieWorldStopped);

    PausePIEHandle = FEditorDelegates::PausePIE.AddRaw(this, &FFrameworkSaveFileEditorModule::OnPieWorldStopped);
    ResumePIEHandle = FEditorDelegates::ResumePIE.AddRaw(this, &FFrameworkSaveFileEditorModule::OnPieWorldStarted);
}

void FFrameworkSaveFileEditorModule::ShutdownModule() {
...
TSharedRef<SDockTab> FFrameworkSaveFileEditorModule::OnSpawnPluginTab(const FSpawnTabArgs& SpawnTabArgs) {
    // Create SDockTab
    ...
    PluginCommands->MapAction(
        ...
        FExecuteAction::CreateStatic(
            &FFrameworkSaveFileEditorModule::ToggleTabVisibility,
            SaveEditorTabManagerWeak,
            SaveEditorManagerName),
        FCanExecuteAction::CreateStatic(
            []() { return true; }),
        FIsActionChecked::CreateStatic(
            &FFrameworkSaveFileEditorModule::IsTabVisible,
            SaveEditorTabManagerWeak,
            SaveEditorManagerName));
...
void FFrameworkSaveFileEditorModule::OnPieWorldStarted(bool bIsSimulating) {
    bGameStarted = true;
}

void FFrameworkSaveFileEditorModule::OnPieWorldStopped(bool bIsSimulating) {
    bGameStarted = false;
}

void FFrameworkSaveFileEditorModule::PluginButtonClicked() {
    FGlobalTabmanager::Get()->TryInvokeTab(SaveFileEditorTabName);
}

bool FFrameworkSaveFileEditorModule::GetSaveGameNames(TSet<FString>& FoundSaves, FString SaveDirectory) {
...
void FFrameworkSaveFileEditorModule::RegisterMenus() {
    FToolMenuOwnerScoped OwnerScoped(this);
    {
        ...
        if (!HubberToolsMenu) {
            ...
            HubberToolsMenu = MainMenu->AddSubMenuScript(FName("HubberTools"), FName("Tools"), FName("HubberTools"), FText::FromString(TEXT("Framework Tools")));
        }
...
IMPLEMENT_MODULE(FFrameworkSaveFileEditorModule, FrameworkSaveFileEditor)