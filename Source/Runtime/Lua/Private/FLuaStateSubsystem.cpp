...
const char* FLuaStateSubsystem::StartLuaPandaFunction = "StartLuaPanda";
...
const char FLuaStateSubsystem::PluginOrGameFeatureFilePathMark = '$';

#if WITH_EDITOR
const char FLuaStateSubsystem::DevelopersFilePathMark = '@';
#endif

const FString FLuaStateSubsystem::LuaInGameFilePath = TEXT("Program/Lua");
...
FLuaStateSubsystem::FLuaStateSubsystem()
	: bHasInit(false), bHasStart(false), State(nullptr), gameInstance(nullptr), _timerindex(0)
{
}
...
void FLuaStateSubsystem::CreateLuaState(UGameInstance* Instance)
{
	...
	// 设置路径
	LuaFrameworkMain = "LuaFramework/Main";
...
TArray<uint8> FLuaStateSubsystem::LoadLuaFile(const char* InFn, FString& OutFilePath)
{
	...
	FString Path;
	// 项目内的Lua脚本(不带有标识前缀)
	if (FirstMask != PluginOrGameFeatureFilePathMark)
	{
		Path = FPaths::ProjectContentDir() /= LuaInGameFilePath;
	}
#if WITH_EDITOR
	// 自定义Lua脚本的路径(带有@标识前缀)
	else if (FirstMask != DevelopersFilePathMark)
	{
...
void FLuaStateSubsystem::Start()
{
	...
	UE_LOG(LogFrameworkLua, Warning, TEXT("lua run begin"));
	// 载入框架文件
	State->doFile(TCHAR_TO_ANSI(*LuaFrameworkMain));
	// 如果是编辑器模式，可以连接 IED 的 Debug Server
	if (FFrameworkCommonLibrary::WithInEditor())
	{
		// 通过Lua状态机调用_G.StartLuaPanda函数
		State->call(StartLuaPandaFunction, LuaPandaConnectorConfig.IP, LuaPandaConnectorConfig.Port);
	}
	// 通过Lua状态机调用_G.Main函数
...
void FLuaStateSubsystem::SetMainFile(const FString& MainFileValue)
{
	LuaFrameworkMain = MainFileValue;
}
