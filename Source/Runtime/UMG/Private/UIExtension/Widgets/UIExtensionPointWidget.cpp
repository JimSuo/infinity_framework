...
TSharedRef<SWidget> UUIExtensionPointWidget::RebuildWidget()
{
	if (!IsDesignTime() && ExtensionPointTag.IsValid())
	{
		...
		FDelegateHandle Handle = GetOwningLocalPlayer<UFrameworkLocalPlayer>()->CallAndRegister_OnPlayerStateSet(
			UFrameworkLocalPlayer::FPlayerStateSetDelegate::FDelegate::CreateUObject(this, &UUIExtensionPointWidget::RegisterExtensionPointForPlayerState)
		);
	}
...
void UUIExtensionPointWidget::RegisterExtensionPointForPlayerState(UFrameworkLocalPlayer* LocalPlayer, APlayerState* PlayerState)
{
	if (UUIExtensionSubsystem* ExtensionSubsystem = GetWorld()->GetSubsystem<UUIExtensionSubsystem>())
	{
		TArray<UClass*> AllowedDataClasses;
