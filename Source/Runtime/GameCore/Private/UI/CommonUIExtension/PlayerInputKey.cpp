...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PlayerInputKey)
...
UPlayerInputKey::UPlayerInputKey(const FObjectInitializer& ObjectInitializer)
...
void UPlayerInputKey::NativePreConstruct()
{
...
void UPlayerInputKey::NativeConstruct()
{
	Super::NativeConstruct();
}

void UPlayerInputKey::NativeDestruct()
{
...
int32 UPlayerInputKey::NativePaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
void UPlayerInputKey::StartHoldProgress(FName HoldActionName, float HoldDuration)
{
...
void UPlayerInputKey::StopHoldProgress(FName HoldActionName, bool bCompletedSuccessfully)
{
...
void UPlayerInputKey::SyncHoldProgress()
{
...
void UPlayerInputKey::UpdateHoldProgress()
{
...
void UPlayerInputKey::UpdateKeybindWidget()
{
	if (!GetOwningPlayer<AFrameworkPlayerController>())
	{
	...
	UE_LOG(LogPlayerInput, Verbose, TEXT("UCommonKeybindWidget::UpdateKeybindWidget: Action: %s Key: %s"), *(BoundAction.ToString()), *(BoundKey.ToString()));
...
void UPlayerInputKey::SetBoundKey(FKey NewKey)
{
	if (NewKey != BoundKey)
	{
...
void UPlayerInputKey::SetBoundAction(FName NewBoundAction)
{
...
void UPlayerInputKey::NativeOnInitialized()
{
	Super::NativeOnInitialized();

	if (auto* LocalPlayer = GetOwningLocalPlayer<UFrameworkLocalPlayer>())
	{
		LocalPlayer->OnPlayerControllerSet.AddUObject(this, &ThisClass::HandlePlayerControllerSet);
	}
}

void UPlayerInputKey::SetForcedHoldKeybind(bool InForcedHoldKeybind)
{
...
void UPlayerInputKey::SetForcedHoldKeybindStatus(ECommonKeybindForcedHoldStatus InForcedHoldKeybindStatus)
{
...
void UPlayerInputKey::SetShowProgressCountDown(bool bShow)
{
	bShowTimeCountDown = bShow;
}

void UPlayerInputKey::SetupHoldKeybind()
{
	auto* OwningPC = Cast<AFrameworkPlayerController>(GetOwningPlayer());
...
void UPlayerInputKey::ShowHoldBackPlate()
{
	bool bDirty = false;
...
void UPlayerInputKey::HandlePlayerControllerSet(UFrameworkLocalPlayer* LocalPlayer, APlayerController* PlayerController)
{
	if (bWaitingForPlayerController && GetOwningPlayer<AFrameworkPlayerController>())
	{
		UpdateKeybindWidget();
...
void UPlayerInputKey::RecalculateDesiredSize()
{
